...
impl RU256 {
        ...
        if over {
            x3 = x3.checked_add(
                U256::MAX.checked_sub(p.v).expect("sub")
                    .checked_add(U256::from_big_endian(&[1])).expect("convesion")
            ).expect("add");
        }
    ...
    pub fn exp_mod(&self, e: &Self, p: &Self) -> Self {
        let seq = e.clone();
        let multiplier = RU256{ v: self.v.checked_rem(p.v).expect("modulo") };
...
mod tests {
    ...
    fn ru256_addition_case_3() {
        let a = RU256::from_str("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2E").unwrap();
        let b = RU256::from_str("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2E").unwrap();
        let p = RU256::from_str("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F").unwrap();
